/*!
 * \ingroup td_3_group
 * \file readpipeclosed.c
 * \brief Exercise 3.4
 *
 * Creates a process and accesses the write side of the pipe while
 * the parent process has closed the the read side.
 *
 ...
 *
 * Creates a process and accesses the read side of the pipe while
 * the parent process has closed the write side of the pipe.
 *
 * This program uses the following system calls: 
 * 
 *  - [pipe(int pipefd[2])](https://man7.org/linux/man-pages/man2/pipe.2.html)
...
int main(void)
{
    ...
    if (!child_pid)
    {
        ...
        /* Infinite loop and does not read once the pipe closed */

        while ((rw_result = read(fd[0], &c[0], sizeof(char))) != -1)
        {  
            count += rw_result;
            sprintf(total, "Read  [%2ld/10]: ", count);
    ...
    else
    {
        ...
        while (i)
        {
            ...
            count += rw_result;
            sprintf(total, "Write [%2ld/10]: ", count);
