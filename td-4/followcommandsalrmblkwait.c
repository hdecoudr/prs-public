/*!
 * \ingroup td_4_group
 * \file followcommandsalrmblkwait.c
 * \brief Exercise 4.6
 * 
...
int main(int argc, char** argv)
{
    ...
    int result = sigemptyset(&act.sa_mask);
    ...
    */
    ...
    result = sigemptyset(&current_mask);
    exit_on_error(result < 0);
    
    /* 
        Add SIGALRM to the mask

        Do not handle SIGALRM
     */

    result = sigaddset(&current_mask, SIGALRM);
    exit_on_error(result < 0);

    /* 
        Do not handle SIGALRM 

        Will not execute SIGALRM 
    */

    result = sigprocmask(SIG_BLOCK, &current_mask, &old_mask);
    exit_on_error(result < 0);
